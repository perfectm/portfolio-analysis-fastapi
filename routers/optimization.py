...
def safe_float(value, default=0.0):
    """Safely convert a value to float, handling None and missing values"""
    if value is None:
        return default
    try:
        return float(value)
...
async def analyze_selected_portfolios_weighted(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        for portfolio_id in valid_portfolio_ids:
            ...
            portfolios_data.append((portfolio_id, portfolio.name, df))  # Store portfolio_id with data
            ...
            if 'metrics' in result:
                ...
                if 'clean_df' in result and not is_single_day:
                    try:
                        ...
                        plot_paths = create_plots(
                            result['clean_df'],
                            result['metrics'],
                            filename_prefix=f"weighted_analysis_portfolio_{i}_{result.get('portfolio_id', 'unknown')}",
                            sma_window=20
                        )
                ...
                simplified_result = {
                    ...
                    'metrics': {
                        ...
                        'ulcer_index': safe_float(result['metrics'].get('ulcer_index'), 0),
                        'upi': safe_float(result['metrics'].get('upi'), 0),
                        'kelly_criterion': safe_float(result['metrics'].get('kelly_criterion'), 0),
                        'cvar': safe_float(result['metrics'].get('cvar'), 0),
                        'total_return': safe_float(result['metrics'].get('total_return'), 0),
                        'total_pl': safe_float(result['metrics'].get('total_pl'), 0),
                        'final_account_value': safe_float(result['metrics'].get('final_account_value'), 100000),
                        'max_drawdown': safe_float(result['metrics'].get('max_drawdown'), 0),
                        'max_drawdown_percent': safe_float(result['metrics'].get('max_drawdown_percent'), 0),
                        'max_drawdown_date': result['metrics'].get('max_drawdown_date', ''),
                        'cagr': safe_float(result['metrics'].get('cagr'), 0),
                        'annual_volatility': safe_float(result['metrics'].get('annual_volatility'), 0),
                        'mar_ratio': safe_float(result['metrics'].get('mar_ratio'), 0),
                        'time_period_years': safe_float(result['metrics'].get('time_period_years'), 0),
                        'number_of_trading_days': int(result['metrics'].get('number_of_trading_days') or 0),
                        'worst_pl_day': safe_float(result['metrics'].get('worst_pl_day'), 0),
                        'worst_pl_date': result['metrics'].get('worst_pl_date', ''),
                        'best_pl_day': safe_float(result['metrics'].get('best_pl_day'), 0),
                        'best_pl_date': result['metrics'].get('best_pl_date', ''),
                        'days_in_drawdown': int(result['metrics'].get('days_in_drawdown') or 0),
                        'avg_drawdown_length': safe_float(result['metrics'].get('avg_drawdown_length'), 0),
                        'num_drawdown_periods': int(result['metrics'].get('num_drawdown_periods') or 0),
                        'days_loss_over_half_pct': int(result['metrics'].get('days_loss_over_half_pct') or 0),
                        'days_loss_over_three_quarters_pct': int(result['metrics'].get('days_loss_over_three_quarters_pct') or 0),
                        'days_loss_over_one_pct': int(result['metrics'].get('days_loss_over_one_pct') or 0),
                        'days_gain_over_half_pct': int(result['metrics'].get('days_gain_over_half_pct') or 0),
                        'days_gain_over_three_quarters_pct': int(result['metrics'].get('days_gain_over_three_quarters_pct') or 0),
                        'days_gain_over_one_pct': int(result['metrics'].get('days_gain_over_one_pct') or 0),
                        'days_loss_over_half_pct_starting_cap': int(result['metrics'].get('days_loss_over_half_pct_starting_cap') or 0),
                        'days_loss_over_three_quarters_pct_starting_cap': int(result['metrics'].get('days_loss_over_three_quarters_pct_starting_cap') or 0),
                        'days_loss_over_one_pct_starting_cap': int(result['metrics'].get('days_loss_over_one_pct_starting_cap') or 0),
                        'days_gain_over_half_pct_starting_cap': int(result['metrics'].get('days_gain_over_half_pct_starting_cap') or 0),
                        'days_gain_over_three_quarters_pct_starting_cap': int(result['metrics'].get('days_gain_over_three_quarters_pct_starting_cap') or 0),
            ...
            try:
                ...
                if blended_df is not None and blended_metrics is not None:
                    ...
                    if portfolio_weights:
                        for i, (_, name, _df) in enumerate(portfolios_data):
                    ...
                    simplified_blended_result = {
                        ...
                        'metrics': {
                            'sharpe_ratio': safe_float(blended_metrics.get('sharpe_ratio'), 0),
                            'sortino_ratio': safe_float(blended_metrics.get('sortino_ratio'), 0),
                            'ulcer_index': safe_float(blended_metrics.get('ulcer_index'), 0),
                            'upi': safe_float(blended_metrics.get('upi'), 0),
                            'kelly_criterion': safe_float(blended_metrics.get('kelly_criterion'), 0),
                            'cvar': safe_float(blended_metrics.get('cvar'), 0),
                            'total_return': safe_float(blended_metrics.get('total_return'), 0),
                            'total_pl': safe_float(blended_metrics.get('total_pl'), 0),
                            'final_account_value': safe_float(blended_metrics.get('final_account_value'), 100000),
                            'max_drawdown': safe_float(blended_metrics.get('max_drawdown'), 0),
                            'max_drawdown_percent': safe_float(blended_metrics.get('max_drawdown_percent'), 0),
                            'max_drawdown_date': blended_metrics.get('max_drawdown_date', ''),
                            'cagr': safe_float(blended_metrics.get('cagr'), 0),
                            'annual_volatility': safe_float(blended_metrics.get('annual_volatility'), 0),
                            'mar_ratio': safe_float(blended_metrics.get('mar_ratio'), 0),
                            'time_period_years': safe_float(blended_metrics.get('time_period_years'), 0),
                            'number_of_trading_days': int(blended_metrics.get('number_of_trading_days') or 0),
                            'beta': safe_float(blended_metrics.get('beta'), 0),
                            'alpha': safe_float(blended_metrics.get('alpha'), 0),
                            'r_squared': safe_float(blended_metrics.get('r_squared'), 0),
                            'beta_observation_count': int(blended_metrics.get('beta_observation_count') or 0),
                            'worst_pl_day': safe_float(blended_metrics.get('worst_pl_day'), 0),
                            'worst_pl_date': blended_metrics.get('worst_pl_date', ''),
                            'best_pl_day': safe_float(blended_metrics.get('best_pl_day'), 0),
                            'best_pl_date': blended_metrics.get('best_pl_date', ''),
                            'days_in_drawdown': int(blended_metrics.get('days_in_drawdown') or 0),
                            'avg_drawdown_length': safe_float(blended_metrics.get('avg_drawdown_length'), 0),
                            'num_drawdown_periods': int(blended_metrics.get('num_drawdown_periods') or 0),
                            'days_loss_over_half_pct': int(blended_metrics.get('days_loss_over_half_pct') or 0),
                            'days_loss_over_three_quarters_pct': int(blended_metrics.get('days_loss_over_three_quarters_pct') or 0),
                            'days_loss_over_one_pct': int(blended_metrics.get('days_loss_over_one_pct') or 0),
                            'days_gain_over_half_pct': int(blended_metrics.get('days_gain_over_half_pct') or 0),
                            'days_gain_over_three_quarters_pct': int(blended_metrics.get('days_gain_over_three_quarters_pct') or 0),
                            'days_gain_over_one_pct': int(blended_metrics.get('days_gain_over_one_pct') or 0),
                            'days_loss_over_half_pct_starting_cap': int(blended_metrics.get('days_loss_over_half_pct_starting_cap') or 0),
                            'days_loss_over_three_quarters_pct_starting_cap': int(blended_metrics.get('days_loss_over_three_quarters_pct_starting_cap') or 0),
                            'days_loss_over_one_pct_starting_cap': int(blended_metrics.get('days_loss_over_one_pct_starting_cap') or 0),
                            'days_gain_over_half_pct_starting_cap': int(blended_metrics.get('days_gain_over_half_pct_starting_cap') or 0),
                            'days_gain_over_three_quarters_pct_starting_cap': int(blended_metrics.get('days_gain_over_three_quarters_pct_starting_cap') or 0),
