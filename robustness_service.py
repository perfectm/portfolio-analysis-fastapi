...
class RobustnessTestService:
    ...
    def _generate_random_periods(
        ...
        The last period will always be the most recent period (working backwards from max_date).
        The remaining periods will be randomly generated.
        """
        ...
        # Create the most recent period (working backwards from max_date)
        most_recent_start = max_date - timedelta(days=period_length_days)
        most_recent_period = (most_recent_start, max_date)
        logger.info(f"Most recent period: {most_recent_start.date()} to {max_date.date()} ({period_length_days} calendar days)")

        # Generate random periods (num_periods - 1 random periods)
        periods = []
        num_random_periods = num_periods - 1

        # Calculate the total range of possible start dates
        total_range_days = (latest_start_date - min_date).days

        for i in range(num_random_periods):
            max_attempts_per_period = 100
            period_found = False

            for attempt in range(max_attempts_per_period):
                ...
                # Ensure end date doesn't exceed max_date
                if period_end_date > max_date:
                    period_end_date = max_date
                    random_start_date = max_date - timedelta(days=period_length_days)

                # Check if this period contains at least 1 trade
                trade_count = self.db.query(PortfolioData).filter(
                    PortfolioData.portfolio_id == portfolio_id,
                    PortfolioData.date >= random_start_date,
                    PortfolioData.date <= period_end_date
                ).count()

                if trade_count > 0:
                    periods.append((random_start_date, period_end_date))
                    logger.info(f"Added random period {i+1}/{num_random_periods}: {random_start_date.date()} to {period_end_date.date()} ({period_length_days} calendar days, {trade_count} trades)")
                    period_found = True
                    break
                else:
                    logger.debug(f"Period {random_start_date.date()} to {period_end_date.date()} has no trades, retrying... (attempt {attempt+1}/{max_attempts_per_period})")

            if not period_found:
                logger.warning(f"Could not find a period with trades after {max_attempts_per_period} attempts for period {i+1}/{num_random_periods}")

        # Add the most recent period as the last period
        periods.append(most_recent_period)

        # Sort periods chronologically by start date
        periods.sort(key=lambda period: period[0])

        logger.info(f"Generated {len(periods)} periods for portfolio {portfolio_id} ({num_random_periods} random + 1 most recent)")
        return periods
    ...
    def _analyze_period(
        ...
        result = {
            ...
            'max_drawdown_percent': safe_float(metrics.get('max_drawdown_percent')),
            ...
            'avg_trade_return': safe_float(metrics.get('avg_trade_return')),
            'total_return': safe_float(metrics.get('total_return')),
            'total_pl': safe_float(metrics.get('total_pl')),
            'final_account_value': safe_float(metrics.get('final_account_value'), starting_capital),
            'ulcer_index': safe_float(metrics.get('ulcer_index')),
            'upi': safe_float(metrics.get('upi')),
            'kelly_criterion': safe_float(metrics.get('kelly_criterion')),
            'mar_ratio': safe_float(metrics.get('mar_ratio')),
            'pcr': safe_float(metrics.get('pcr')),
            'cvar': safe_float(metrics.get('cvar')),
    ...
    def _calculate_statistics(
        ...
        for metric in metrics_to_analyze:
            ...
            logger.info(f"[Statistics] Metric '{metric}': Found {len(values)} period values (before filtering)")
            values = [v for v in values if not pd.isna(v) and not np.isinf(v)]
            logger.info(f"[Statistics] Metric '{metric}': {len(values)} valid values after filtering")

            if not values:
                logger.warning(f"[Statistics] Metric '{metric}': No valid values, skipping")
                continue

            values = np.array(values)
            full_value = full_metrics.get(metric, 0)
            logger.info(f"[Statistics] Metric '{metric}': Calculating statistics with full_value={full_value}")
