...
const RegimeAnalysis: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [historyResponse, portfoliosResponse] = await Promise.all([
        ...
        api.get<Portfolio[]>('/api/strategies'),
      ]);

      setRegimeHistory(historyResponse.data);
      setPortfolios(portfoliosResponse.data);

      // Auto-select first few portfolios
      const defaultSelection = portfoliosResponse.data.slice(0, 3).map(p => p.id);
      setSelectedPortfolios(defaultSelection);

      // Fetch performance data for all portfolios
      if (portfoliosResponse.data.length > 0) {
        await fetchPortfolioPerformances(portfoliosResponse.data.map(p => p.id));
      }
    } catch (err: any) {
  ...
  const fetchPortfolioPerformances = async (portfolioIds: number[]) => {
    try {
      ...
      setPortfolioPerformances(responses.map(r => r.data));
    } catch (err) {
                        ...
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                            <Chip
                              ...
                              color={getRegimeColor(row.regime) as any}
                              size="small"
                            />
                          </TableCell>
                            ...
                            <TableRow key={`${perf.portfolio_id}-${regime}`}>
                              ...
                              <TableCell>
                                <Chip
                                  ...
                                  color={getRegimeColor(regime) as any}
                                  size="small"
                                />
                              </TableCell>
                              <TableCell align="right">
